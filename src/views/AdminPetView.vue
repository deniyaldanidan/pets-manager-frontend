...
const acceptFN = async () => {
    try {
        const res = await myAxios.put(`/pets-management/approve/${petId}`, undefined, { headers: { Authorization: token } });
        if (res.data?.approved === true && typeof res.data?.approved_at === 'string' && petData.value) {
            petData.value.approved = true;
            petData.value.approved_at = res.data.approved_at
        }
    } catch (error) {
        console.log(error);
    }
...
<template>
    <div class=" px-myspace py-myspace max-w-[1250px] mx-auto">
        <PetInfoDetailedComp v-if="petData !== null" :type="petData.type" :name="petData.name"
            ...
            <div class="flex gap-x-14">
                <button class="action-btns bg-primary" @click="acceptFN" v-if="!petData.approved">Approve</button>
            ...
            <Teleport to="#portal1">
                <div v-if="deletePopState"
                    ...
                    <div class="min-h-40 px-4 py-4 text-lg bg-red-100">Are you sure you want to delete? This action is
                        not
                        reversible.
                    </div>
                    <div
